...
/** 章序号按比例从旧形态映射到新形态 */
function scaleIndex(index, fromCount, toCount) {
...
/** 迁移时把旧进度换算成目标书的章序号 */
function planMigrate(src, titles) {
...
class NovelReaderView extends obsidian_1.ItemView {
    ...
    async openSource(src) {
        ...
        if (progress && progress.overallOnly && this.chapters.length > 0) {
            ...
            index = Math.min(this.chapters.length - 1, Math.max(0, Math.round((progress.overall / 100) * this.chapters.length) - 1));
            target = { page: 0 };
    ...
    buildShell() {
        ...
        this.registerDomEvent(exit, 'click', (evt) => {
            evt.stopPropagation();
    ...
    onOpen() {
        ...
        mk('迁移进度到…', '', () => {
            new PickBookModal(plugin, entry.key, (target) => {
                new ConfirmModal(this.app, '迁移进度', `把《${entry.title}》的进度、书签、收藏与显示名迁到《${target}》？\n` +
                    `定位方式：优先按章号匹配，匹配不上按全书百分比估算；源条目会从书架撤下。`, () => {
                    const r = plugin.migrateBook(entry.key, target);
                    const how = r.how === 'number' ? '按章号命中' : r.how === 'percent' ? '按百分比估算' : '目标章数未知，打开时再定位';
                    new obsidian_1.Notice(`已迁移到《${target}》：第 ${r.chapter + 1} 章（${how}）` +
        ...
        mk('从书架隐藏', '', () => {
            ...
            new obsidian_1.Notice(`《${entry.title}》已从书架隐藏，进度仍保留（设置里可恢复）`);
            this.onChanged();
            this.close();
        });
        mk('移除', 'nr-btn-danger', () => {
...
class PickBookModal extends obsidian_1.FuzzySuggestModal {
    constructor(plugin, exclude, onPick) {
        super(plugin.app);
        this.plugin = plugin;
    ...
    getItems() {
        return collectLibrary(this.app, this.plugin.data.settings.deepBrowse).filter((item) => itemPath(item) !== this.exclude);
    }
    getItemText(item) {
...
class HiddenBooksModal extends obsidian_1.Modal {
    constructor(plugin) {
        super(plugin.app);
        this.plugin = plugin;
    }
    onOpen() {
        ...
        const keys = Object.keys(this.plugin.data.shelfMeta).filter((k) => this.plugin.data.shelfMeta[k] && this.plugin.data.shelfMeta[k].hidden);
        if (keys.length === 0) {
        ...
        for (const key of keys) {
            const af = this.app.vault.getAbstractFileByPath(key);
            const name = af instanceof obsidian_1.TFolder ? af.name : af instanceof obsidian_1.TFile ? af.basename : key;
            ...
            show.onclick = () => {
                ...
                new obsidian_1.Notice(`《${meta.alias || name}》已恢复显示`);
            ...
            del.onclick = () => {
                new ConfirmModal(this.app, '彻底移除', `移除《${name}》的阅读记录与书签？`, () => {
                    ...
                    new obsidian_1.Notice(`已移除《${name}》`);
                    this.plugin.refreshShelfViews();
                    this.close();
                }).open();
            };
...
class NovelReaderSettingTab extends obsidian_1.PluginSettingTab {
    ...
    display() {
        ...
        const hiddenCount = Object.keys(this.plugin.data.shelfMeta).filter((k) => this.plugin.data.shelfMeta[k] && this.plugin.data.shelfMeta[k].hidden).length;
        new obsidian_1.Setting(containerEl)
            .setName('已隐藏的书')
